...
This repo defines and evolves the **Open Knowledge Format (OKF)** (`WIKI_SPEC.md`, version 0.2), holds a real OKF knowledge bundle (`vault/`), and holds the local-only tool that produces and serves that bundle (`pipeline/`).
...
Three things live here, each with a different scope:
...
- **`pipeline/`** — a hexagonal-architecture Python tool (Ollama + ChromaDB + SQLite, all local) that watches `vault/raw/`, turns unprocessed notes/documents into conformant concepts in `vault/`, indexes them for semantic search, and serves the vault to MCP clients (e.g. Claude) over a stateless Streamable HTTP MCP server. It has its own docs — start at `pipeline/README.md`, then `pipeline/docs/` (an MkDocs site: architecture, domain model, CLI/MCP/config reference, dev onboarding). Treat those as authoritative for anything about the pipeline's code; this file only covers the OKF spec and vault conventions.
